...
void arcsec2meter(float lat,float *as2m)
   {
   const float radius=6370997.0f;

   if (fabs(lat)>90*60*60) ERRORMSG();

   as2m[1]=2*PI*radius/(360*60*60);
   as2m[0]=as2m[1]*fcos(lat*2*PI/(360*60*60));
   }
...
unsigned char *normalize(unsigned char *hfield,
                         int width,int height,int components,
                         float extentx,float extenty,
                         float scaling,
                         int missing)
