...
static void initglexts()
   ...
   if (!done)
      ...
      if (strstr(GL_EXTs,"GL_EXT_texture_compression_s3tc")!=NULL) glext_s3=TRUE;
...
int buildtexmap(unsigned char *image,int *width,int *height,int components,int depth,int mipmaps,int s3tc,int bytes)
   ...
   switch (components)
      ...
      case 3:
         ...
         else if (glext_s3) texsource=GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
#endif
         else ERRORMSG();
         break;
      case 4:
         ...
         else if (glext_s3) texsource=GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
#endif
         else ERRORMSG();
         break;
      default: ERRORMSG();
   ...
   if (s3tc!=0 && !glext_s3) ERRORMSG();
#ifdef GL_ARB_texture_compression
   else if (s3tc!=0) glCompressedTexImage2DARB(GL_TEXTURE_2D,0,texsource,*width,*height,0,bytes,image3);
#endif
   else if (mipmaps!=0) gluBuild2DMipmaps(GL_TEXTURE_2D,texformat,*width,*height,texsource,GL_UNSIGNED_BYTE,image3);
