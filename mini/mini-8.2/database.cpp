...
// magic identifier
unsigned int databuf::MAGIC1=12640; // original magic identifier
unsigned int databuf::MAGIC2=13048; // backwards compatibility
unsigned int databuf::MAGIC3=13091; // actual magic identifier
...
void (*databuf::INTERPRETER_INIT)(unsigned int implformat,char *program,databuf *obj,void *data)=NULL;
...
void databuf::writeparam(char *tag,float v,FILE *file,int digits)
   ...
   while (10.0f*point<=value) point*=10.0f;

   if (value<1.0f) putc('0',file);
   else
      while (point>=1.0f)
         {
         digit=ftrc(value/point);
   ...
   if (value>0.0f)
      ...
      while (value>0.0f && digits>0)
         {
         if (digits>1) digit=ftrc(value/point);
         else digit=ftrc(value/point+0.5f);
...
int databuf::readparam(char *tag,float *v,FILE *file)
   ...
   if (ch=='e' || ch=='E')
      ...
      while (ch>='0' && ch<='9')
         {
         expnt=10.0f*expnt+ch-'0';
         ch=getc(file);
         }
      }
   else expnt=exsgn=0.0f;

   while (ch==' ' || ch=='\n' || ch=='\r') ch=getc(file);

   ungetc(ch,file);

   *v=value/point*fpow(10.0f,exsgn*expnt);
...
int databuf::loaddata(const char *filename)
   ...
   else if (m!=MAGIC1 && m!=MAGIC2 && m!=MAGIC3)
      {
      fclose(file);
      return(0);
      }
...
void databuf::interpretechunk(unsigned int implfmt)
   ...
   INTERPRETER_INIT(implfmt,(char *)data,this,INTERPRETER_DATA);
   ...
   for (x=0; x<xsize; x++)
      for (y=0; y<ysize; y++)
         for (z=0; z<zsize; z++)
            for (t=0; t<tsteps; t++)
               {
               if (xsize>0) xx=(float)x/(xsize-1);
               else xx=0.0f;

               if (ysize>0) yy=(float)y/(ysize-1);
               else yy=0.0f;

               if (zsize>0) zz=(float)z/(zsize-1);
               else zz=0.0f;

               if (tsteps>0) tt=(float)t/(tsteps-1);
               else tt=0.0f;
...
void databuf::setinterpreter(void (*parser)(unsigned int implformat,char *program,databuf *obj,void *data),void *data,
...
int databuf::loadPVMdata(const char *filename,
   ...
   for (i=t+1; i<t+n; i++)
      ...
      if (width!=xsize || height!=ysize || depth!=zsize || components!=1) ERRORMSG();
...
void databuf::convertdata(unsigned int newtype)
   ...
   if (type==0 && newtype==1)
      ...
      if (scaling!=1.0f || bias!=0.0f)
         for (count=0; count<cells; count++) *floatptr++=scaling*(*floatptr)+bias;
