...
float minitile::CONFIGURE_GRADINF=0.25f;
         ...
         if (hfields!=NULL)
            ...
            if (fogmaps!=NULL)
               if (fogmaps[i+j*cols]!=NULL)
                  ...
                  FOGMAP[i+j*cols]=mini::initfogmap(image,width,lambda,displace,emission,attenuation,fogR,fogG,fogB);
                  if (loaded==0) free(image);
                  }
            }
...
void minitile::drawtile(float res,
   ...
   if (phase==2)
      if (TEXMAP_CALLBACK!=NULL)
         TEXMAP_CALLBACK(col,row,SIZE[col+row*COLS]);
...
void minitile::draw(float res,
   ...
   if (PHASE<0 || update==0) steps=2*width*height;
   else steps=(2*width*height+update-1)/update;
   ...
   for (i=0; i<steps; i++)
      ...
      if (PHASE!=2 || UPDATED[col+row*COLS]==0)
         drawtile(res,
                  ex,ey,ez,
                  dx,dy,dz,
                  ux,uy,uz,
      ...
      if (++COL>RIGHT)
         ...
         if (++ROW>TOP)
            ...
            if (TRIGGER_CALLBACK!=NULL) TRIGGER_CALLBACK(PHASE,(PHASE!=3)?SCALE*RELSCALE:LAMBDA,ex,ey,ez);
...
minitile *minitile::load(int cols,int rows,
   ...
   if ((image=readPNMfile(hmaps[0],&width,&height,&components,&comment))==NULL) ERRORMSG();
   free(image);
...
void minitile::reload(int col,int row,
   ...
   if (fmap!=NULL)
      ...
      FOGMAP2[col+row*COLS]=mini::initfogmap(image,width,LAMBDA,DISPLACE,EMISSION,ATTENUATION,FR,FG,FB);
      if (loaded==0) free(image);
      }
...
void minitile::reload(int col,int row,
   ...
   if (hmap.type!=2)
      ...
      if (!fmap.missing())
         ...
         FOGMAP2[col+row*COLS]=mini::initfogmap((unsigned char *)fmap.data,width,LAMBDA,DISPLACE,EMISSION,ATTENUATION,FR,FG,FB);
         }
      }
   else
      {
...
void minitile::configure_gradinf(float gradinf)
   {
   if (gradinf<0.0f) gradinf=0.25f;
   CONFIGURE_GRADINF=gradinf;
   }

void minitile::configure_bsafety(float bsafety)
   {
